import assertRaw = require('assert');
import cacher = require('../lazy-memoize');
import sms = require('source-map-support');
const assert = (assertRaw as any).strict as typeof assertRaw;
sms.install();
...
function runTests(options: cacher.Options) {
  describe('cacher', () => {
    ...
    it('should work with a function that takes multi level arguments', async () => {
      ...
      const f = async <A1 extends number, A2 extends number, A3 extends number>(
        n1: A1,
        n2: A2,
        n3: A3
      ) => {
        if (!callMap.has(n1)) {
          callMap.set(n1, new Map());
        }
        const callMap2 = callMap.get(n1);
        if (!callMap2.has(n2)) {
          callMap2.set(n2, new Map());
        }
...
describe('with errors = passthrough', () => {
  ...
  describe('throwing synchronous function', () => {
    ...
    it('should throw once', async () => {
      await assert.rejects(async () => await c(), MyError);
    });
  ...
  describe('refreshing a throwing asynchronous function', () => {
    ...
    const c = cacher(f, 0);
    it('should throw once', async () => {
      await assert.rejects(() => c(), MyError);
      assert.strictEqual(calls, 1);
      await assert.rejects(() => c(), MyError); // triggers refresh but returns cached value in the meantime
      assert.strictEqual(calls, 1);
    });
